...
describe('[Single Public NFT] Contract deployment', async () => {
    it('Single FA2 public collection contract deployment should succeed', async () => {
        [fa2, _] = await deploy(
            './contracts/single-nft-public/single-nft-fa2-public-collection.arl',
            {
                parameters: {
                    owner: alice.pkh,
                },
...
describe('[Single Public NFT] Minting', async () => {
    it('Mint tokens on Single FA2 Public collection contract as owner for ourself should succeed', async () => {
        await fa2.mint({
            arg: {
                itokenid: tokenId,
                iowner: alice.pkh,
    ...
    it('Mint tokens on Single FA2 Public collection contract as non owner for ourself should succeed', async () => {
        await fa2.mint({
            arg: {
                itokenid: tokenId + 1,
                iowner: bob.pkh,
        ...
        var balance = await getValueFromBigMap(
            parseInt(storage.ledger),
            exprMichelineToJson(`${tokenId + 1}`),
            exprMichelineToJson(`nat`)
        );
        assert(balance.string == bob.pkh);
    });

    it('Mint tokens on Single FA2 Public collection contract as non owner for someone else should succeed', async () => {
        await fa2.mint({
            arg: {
                itokenid: tokenId + 2,
                iowner: carl.pkh,
        ...
        var balance = await getValueFromBigMap(
            parseInt(storage.ledger),
            exprMichelineToJson(`${tokenId + 2}`),
            exprMichelineToJson(`nat`)
        );
        assert(balance.string == carl.pkh);
    });

    it('Mint tokens on Single FA2 Public collection contract as owner for someone else should succeed', async () => {
        await fa2.mint({
            arg: {
                itokenid: tokenId + 3,
                iowner: carl.pkh,
    ...
    it('Re-Mint same tokens on Single FA2 Public collection contract should fail', async () => {
...
describe('[Single Public NFT] Update operators', async () => {
...
describe('[Single Public NFT] Add permit', async () => {
...
describe('[Single Public NFT] Transfers', async () => {
    ...
    it('Transfer tokens without an operator and a valid permit (permit consumed)', async () => {
        ...
        await fa2.transfer({
            arg: {
                txs: [[alice.pkh, [[bob.pkh, tokenId + 10, amount]]]],
            },
            as: bob.pkh,
        });
...
describe('[Single Public NFT] Set metadata', async () => {
...
describe('[Single Public NFT] Set expiry', async () => {
...
describe('[Single Public NFT] Burn', async () => {
...
describe('[Single Public NFT] Transfer ownership', async () => {
