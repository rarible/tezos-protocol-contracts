...
describe('Contract deployments', async () => {
    ...
    it('Feeless Sales contract deployment should succeed', async () => {
        [feeless_sales, _] = await deploy(
            './contracts/feeless-sales.arl',
            {
                parameters: {
                    owner: alice.pkh,
                    protocol_fee: 0,
...
describe('(Transfer manager)Authorize Sales, and Sales storage contract tests', async () => {
    it('Authorize Sales, and Sales storage contract as non admin should fail', async () => {
        await expectToThrow(async () => {
            await transfer_manager.authorize_contract({
                arg: {
                    ac_contract: feeless_sales.address
                },
                as: bob.pkh
            });
    ...
    it('Authorize Sales, and Sales storage contract as admin should succeed', async () => {
        ...
        assert(
            ...
            post_test_storage.authorized_contracts.includes(feeless_sales.address)
        );
    });
});
...
describe('Sales storage setter tests', async () => {
    it('Set sales contract as non admin should fail', async () => {
        await expectToThrow(async () => {
            await sales_storage.set_sales_contract({
                arg: {
                    ssc_contract: feeless_sales.address
                },
                as: bob.pkh
            });
        }, errors.INVALID_CALLER);
    ...
    it('Set sales contract as admin should succeed', async () => {
        ...
        assert(post_test_storage.sales_contract == feeless_sales.address);
    });
...
describe('Sales contract setter tests', async () => {
    describe('Sales storage (Sales contract) contract setter tests', async () => {
        it('Set Sales storage contract as non admin should fail', async () => {
            await expectToThrow(async () => {
                await feeless_sales.set_sales_storage({
                    arg: {
        ...
        it('Set Sales storage contract as admin should succeed', async () => {
            const storage = await feeless_sales.getStorage();
            ...
            const post_test_storage = await feeless_sales.getStorage();
    ...
    describe('Protocol fee setter tests', async () => {
        it('Set protocol fee as non admin should fail', async () => {
            await expectToThrow(async () => {
                await feeless_sales.set_protocol_fee({
                    arg: {
                        smd: 999
                    },
                    as: bob.pkh
...
describe('Set feeless sales tests', async () => {
    describe('Set feeless sale in Fungible FA2', async () => {
        it('Set feeless sale buying with Fungible FA2 should succeed (no royalties, no sale payouts, no sale origin fees)', async () => {
            const storage = await sales_storage.getStorage();
            const sale_asset = mkFungibleFA2Asset(fa2_ft.address, token_id_0.toString());
            var sale = await getValueFromBigMap(
                parseInt(storage.sales),
                                        ...
                                        (Pair ${qty}
                                            (Pair None
                                                (Pair None
                                                    (Pair ${max_fees}
                                                        (Pair None None))))))))))))`;
            const sig = await sign(packTyped(exprMichelineToJson(sale_order), sale_type_michelson), { as: alice.name });
            await feeless_sales.sell({
                argMichelson: `
                    (Pair ${sale_order} (Pair "${alice.pubk}" "${sig.prefixSig}"))`,
                as: alice.pkh,
            });
...
//             );
//             assert(sale == null);
//             await feeless_sales.sell_bundle({
//                 argMichelson: `
//                     (Pair 0x${bundle}
//                         (Pair ${parseInt(FA2)}
//                             (Pair 0x${sale_asset}
...
//             const sales_ft_balance = await getFA2Balance(fa2_ft, token_id_0, feeless_sales.address);
...
//             const post_sales_ft_balance = await getFA2Balance(fa2_ft, token_id_0, feeless_sales.address);
...
//             const sales_ft_balance = await getFA2Balance(fa2_ft, token_id_1, feeless_sales.address);
...
//             const post_sales_ft_balance = await getFA2Balance(fa2_ft, token_id_1, feeless_sales.address);
...
//             const sales_ft_balance = await getFA2Balance(fa2_ft, token_id_2, feeless_sales.address);
...
//             const post_sales_ft_balance = await getFA2Balance(fa2_ft, token_id_2, feeless_sales.address);
...
//             const sales_ft_balance = await getBalance(feeless_sales.address);
...
//             const post_sales_ft_balance = await getBalance(feeless_sales.address);
...
//             const sales_ft_balance = await getFA12Balance(fa12_ft_0, feeless_sales.address);
...
//             const post_sales_ft_balance = await getFA12Balance(fa12_ft_0, feeless_sales.address);
...
//             const sales_ft_balance = await getFA12Balance(fa12_ft_1, feeless_sales.address);
...
//             const post_sales_ft_balance = await getFA12Balance(fa12_ft_1, feeless_sales.address);
...
//             const sales_ft_balance = await getFA12Balance(fa12_ft_2, feeless_sales.address);
...
//             const post_sales_ft_balance = await getFA12Balance(fa12_ft_2, feeless_sales.address);
...
//             await feeless_sales.buy_bundle({
//                 argMichelson: `
//                     (Pair 0x${bundle}
//                         (Pair "${alice.pkh}"
//                             (Pair ${parseInt(FA2)}
...
//                 const bundle = mkPackedBundle(bundle_items);
//                 const sale_asset = mkXTZAsset();
//                 await feeless_sales.cancel_bundle_sale({
...
//             await feeless_sales.cancel_sale({
